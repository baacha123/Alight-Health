...
Usage:
    python commcloud_record.py --record              # Start live recording
    python commcloud_record.py --manual              # Create from pasted conversation
    python commcloud_record.py --enhance ./recordings  # Enhance with LLM keywords
    python commcloud_record.py --list ./recordings   # List recorded files
"""
...
def call_gateway_llm(prompt: str, model_id: str, system_prompt: str = None) -> str:
    ...
    if not token or not instance_url:
        raise ValueError("No credentials. Run: orchestrate env activate <env> --api-key <key>")
...
def run_record_command(output_dir: Path):
    ...
    if files:
        ...
        for f in files:
            print(f"  - {f}")
        print(f"\nTo enhance keywords: python commcloud_record.py --enhance {output_dir}")
    else:
...
Examples:
  ...
  orchestrate env activate alight-mvp --api-key <key>
